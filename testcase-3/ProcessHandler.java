...
public class ProcessHandler {
    ...
    public static volatile int p1_flag = 1;
    public static volatile int timeOutFlag = 0;
    ...
    public void manageCoordinateConnection() throws SocketException {
        ...
        try {
            ...
            System.out.println(getTimeStamp() + "  Didn't receive any response from P1!");
            multicastMessageToAllParticipants(LogRecord.GLOBAL_ABORT);
            logRecord.write(LogRecord.GLOBAL_ABORT);
            while(true) {
            }
    ...
    private void handleReceivedMessage(Message receivedMessage, String senderProcessID) {
        if (receivedMessage.getMessage().equals(LogRecord.VOTE_REQUEST)) {
            ...
            System.out.println(getTimeStamp() + "  Received - " + LogRecord.DECISION_REQUEST + " from P1");
            Message message = new Message(logRecord.latest());
            message.setParticipantId("P2");
            sendMessageToParticipant(message, process_portList[1], "P1");
        }
        else {
    ...
    public void sendMessageToCoordinator(Message message, int port) {
        ...
        try {
            socket = new Socket(ProcessHandler.server_host_addr, port);
            ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
            if (pid == 1 && p1_failed_flag == 0) {
                System.out.println(getTimeStamp() + "  This process (P1) has stopped!");
                Thread.sleep(6000);
                System.out.println(getTimeStamp() + "  Recovering from failure...");
                Thread.sleep(5000);
                System.out.println(getTimeStamp() + "  Recovered now from failure!");
                System.out.println(getTimeStamp() + "  Contacting P2 for transaction state...");
                synchronized ((Object) p1_failed_flag) {
                    p1_failed_flag = 1;
                }
                message = new Message(LogRecord.DECISION_REQUEST);
                message.setParticipantId("P2");
                sendMessageToParticipant(message, process_portList[2], "P2");
